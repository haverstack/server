...
describe('Records', () => {
  ...
  describe('POST /records', () => {
    ...
    it('returns 400 when id is missing', async () => {
      const { status } = await req(t.app, 'POST', '/records', {
        token: TEST_TOKEN,
        body: { typeId: NOTE_TYPE_ID, content: { body: 'x' }, version: 1 },
      });
    ...
    it('returns 401 without auth', async () => {
      const { status } = await req(t.app, 'POST', '/records', {
        body: { id: generateId(), typeId: NOTE_TYPE_ID, content: { body: 'x' }, version: 1 },
      });
  ...
  describe('GET /records', () => {
    it('returns records list with total', async () => {
  ...
  describe('PATCH /records/:id', () => {
    it('updates record content', async () => {
      ...
      const { status, data } = await req(t.app, 'PATCH', `/records/${record.id}`, {
        token: TEST_TOKEN,
        body: { content: { body: 'Updated body' }, version: 2, updatedAt: new Date().toISOString() },
      });
      expect(status).toBe(200);
      const d = data as Record<string, unknown>;
