...
| `QUERY_TIMEOUT_MS`       | No        | `10000` (10s)                                | Deadline for a `GET /records` or `POST /records/query` search. Exceeding it answers `503` (code `timeout`) rather than blocking. See [Deployment: bounding query cost](./docs/deployment.md#bounding-query-cost). |
| `QUERY_WORKER_POOL_SIZE` | No        | `2`                                          | Number of worker threads a slow search can run on without blocking other requests. See [Deployment: bounding query cost](./docs/deployment.md#bounding-query-cost).                                               |
