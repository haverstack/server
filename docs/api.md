...
Resume is per-connection state, not global: each distinct (session, filter) pairing gets its own buffer, retained for a bounded time and depth past the last connection using it — long enough for an ordinary reconnect, not indefinitely. How many buffers the server retains at once is bounded too (`CHANGE_BUFFER_LIMIT`), since the pairing that identifies one includes client-supplied query params; past the ceiling the longest-disconnected are dropped early, which their owners see as `cursor_expired`. See [Bounding change-feed cost](./deployment.md#bounding-change-feed-cost) for the three settings and how to size them. A gap resumption can't close is exactly what `reset` exists for; a client's repair is the same either way — reconcile by querying `GET /records` / `POST /records/query`. A `purged` frame in a replayed backlog is never re-checked against current permissions (there is no record left to check, and the mutation-time decision is the only one that will ever exist); every other replayed frame is.
