...
export class AuthNonceStore {
  ...
  redeem(did: string, nonce: string): NonceRedeemResult {
    const row = this.db.prepare('SELECT did, expiresAt FROM nonces WHERE nonce = ?').get(nonce) as
      { did: string; expiresAt: number } | undefined;
    if (!row || row.did !== did) return 'unknown';
   ...
   * need a way to get that value into the store directly.
   */
  seed(nonce: string, did: string, expiresAt: Date): void {
    this.db
