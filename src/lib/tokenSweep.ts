 ...
 * authorization outcome — it reclaims rows, and nothing else does: the
 * store deletes only on an explicit `revokeToken()`. Since `POST
 * /auth/token` is unauthenticated, strangers drive the row count, and
 * unbounded growth costs disk and slows every `listTokens()` caller.
 *
